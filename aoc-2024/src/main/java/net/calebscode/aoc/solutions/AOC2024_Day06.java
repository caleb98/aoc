...
public class AOC2024_Day06 extends BasicSolution<Integer> {
	...
	public Integer solveSecond() {
		...
		for (var step : path) {
			if (!originalObstacles.contains(step.first) && formsLoop(step.first)) {
				newObstacles.add(step.first);
			}
		}
